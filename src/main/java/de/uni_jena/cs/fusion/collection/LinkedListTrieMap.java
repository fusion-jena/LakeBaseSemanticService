package de.uni_jena.cs.fusion.collection;
 ...
 */
public class LinkedListTrieMap<V> implements TrieMap<V> {
	 ...
	 * <dd>The given key is not contained yet. The returned node would be the
	 * parent of the node representing the given key, after it has been
	 * inserted.</dd>
	 * <dt>Given key is a prefix of the key of the returned node</dt>
	 * <dd>The given key is contained, but only as part of an other key and is
	 * not represented by an particular node. The returned node would be a child
	 ...
	 * <dd>The given key is not contained yet. The returned node would be a
	 * sibling of the node representing the given key, after is has been
	 * inserted.</dd>
	 * </dl>
	 * 
	 * @param key
	...
	/**
	 * Refreshes the lengths of the ancestors by collecting their key length and
	 * lengths of their children, including this node.
	 */
	private void refreshAncestorsLengths() {
		if (this.parent != null) {
			this.parent.lengths.clear();
			if (this.parent.isPopulated()) {
				this.parent.lengths.set(this.parent.keyLength());
			}
			for (LinkedListTrieMap<V> child : this.parent.children) {
				this.parent.lengths.or(child.lengths);
			}
			this.parent.refreshAncestorsLengths();
		}
	}
	...
	public Trie<V> getSubTrie(String key) {
		Trie<V> result = getClosestNode(key);
		if (result.key().startsWith(key)) {
			return result;
		} else {
			return (Trie<V>) Tries.emptyTrieMap();
		}
